...
void CPU::executeOpCode(uint8_t opcode){
	...
	// LOADy
	case 0x08: load_SP(); break;// LD (nn), SP - lokacja nn do SP
	// LD REG16, nn - nn do rejestru
	case 0x01: {//BC
	...
	// LD (REG16), REG8 - 8 bitowy rejestr do 16 bitowego
	...
	// LD reg8, n - wartosc do 8 bitowego rejestru 
	...
	// LD REG8, (REG16) - wartosc z rejestru pod lokacje 
	...
	// LD reg8, reg8 - rejestr do rejestru 8 bitowe
	...
	// LD REG8, (HL); - zawartosc rejestru pod lokacje
	...
	// ADC IMM - dodaj carry plus wartosc
	...
	default: {
		cout << "\nNieznana instrukcja: " << hex << static_cast<int>(opcode) << endl;
		cout << "PC: " << pc - 1 << endl;
	}
	}
}
void CPU::extended_opcodes(uint8_t opcode) {
	...
	case 0x18: case 0x19: case 0x1A: case 0x1B: case 0x1C: case 0x1D:  case 0x1F: {
	 ...
	 // EXT SLA - shift lewo arytmetycznie
	...
	//EXT SRA - shift prawo arytmetycznie
	...
	// RLC (HL) - rotacja HL w kólko w lewo
	case 0x06: {
	...
	// RRC (HL) - rotacja HL w kólko w prawo
	case 0x0E: {
		uint8_t data = Memory.readMemory(registerHL.reg);
		rrc_reg8(data);
		if (data == 0) {
	...
	// SLA (HL) - rotacja arytmetyczna HL w lewo
	case 0x26: {
		ext_SLAHL(registerHL.reg);
		cycles += 8;
		break;
	}
	// SRA (HL) - rotacja arytmetyczna HL w prawo
	case 0x2E: {
		ext_SRAHL(registerHL.reg);
		cycles += 8;
		break;
	}
	// lo zmiana z hi bitow
	case 0x36: {
		uint8_t data = Memory.readMemory(registerHL.reg);
		ext_swap(data);
		Memory.writeMemory(registerHL.reg, data);
	...
	// SRL (HL) - rotacja HL logiczna w lewo
	case 0x3E: {
		uint8_t data = Memory.readMemory(registerHL.reg);
		ext_srl(data);
		Memory.writeMemory(registerHL.reg, data);
	...
	default:cout << "\nNieznana instrukcja: " << hex << static_cast<int>(opcode) << endl; break;
	}
}
