...
void GPU::setStatus(){
	...
	if (interrupt && (mode != currMode)) {//zmiana trybu wiec przerwanie
		cpu->setInterrupt(1);
	}
	if (currLine == mem->readMemory(0xFF45)){//flaga przypadku - czyli nastepna linia jest taka sama jak aktulanie wczytywana, i przerwanie by poinformowac o tym gre
...
void GPU::renderSprites(){
	...
	bool use8x16 = false;
	if (cpu->getBit(2, lcdControl)) use8x16 = true;
	for (int sprite = 0; sprite < 40; sprite++) {
		uint8_t index = sprite * 4; // bo 4 bajty zajmuje jeden sprite
		...
		if ((scanLine >= positionY) && (scanLine < (positionY + height))) {
			...
			if (yFlip) {// czytamy od drugiej strony tabeli jesli sprite jest odwrocony 
				line -= height;
				line *= -1;
			}
			line *= 2;// bo dwa bajty zajmuje jedna linia w pamieci
...
void GPU::renderTiles(){
	...
	bool unsig = true;
	if (tileData == 0x8800) {//bity ze znakiem jako identyfikator tilów 
		unsig = false;
	}
	...
	//który z pixeli jest aktualnie rysowany
	uint16_t tileRow = (((uint8_t)(positionY / 8)) * 32);
	// pixele horyzontalnie 
	for (int pixel = 0; pixel < 160; pixel++) {
		...
		line *= 2; //ka¿da linia zajmuje dwa bajty w pamiêci
